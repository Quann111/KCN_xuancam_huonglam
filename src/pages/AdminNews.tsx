...
import { ImagePlus, LogOut, Pencil, Plus, Trash2, Upload, X } from 'lucide-react';
import { formatNewsDate, generateSlug, normalizeSlug } from '../lib/news-utils';
import { NewsPost, NewsStatus, supabase } from '../lib/supabase';

type FormState = {
  title: string; slug: string; excerpt: string; content: string; category: string; cover_image_url: string;
  seo_title: string; seo_description: string; status: NewsStatus;
};

const emptyForm: FormState = { title: '', slug: '', excerpt: '', content: '', category: 'Sự kiện', cover_image_url: '', seo_title: '', seo_description: '', status: 'draft' };
...
export default function AdminNews() {
  ...
  const loadPosts = async () => {
    const { data, error: requestError } = await supabase.from('news_posts').select('*').order('created_at', { ascending: false });
    if (requestError) setError(requestError.message);
    else setPosts((data ?? []) as NewsPost[]);
  };

  const openAdminWorkspace = async () => {
    const { data, error: adminError } = await supabase.rpc('is_news_admin');
  ...
  const checkDuplicateTitle = async (title: string) => {
    ...
    const { data } = await supabase.from('news_posts').select('id, title').eq('slug', slug).maybeSingle();
  ...
  const uploadImage = async (event: ChangeEvent<HTMLInputElement>, inline = false) => {
    ...
    const { error: uploadError } = await supabase.storage.from('news-images').upload(path, file, { cacheControl: '3600', upsert: false });
  ...
  const savePost = async (event: FormEvent) => {
    ...
    const payload = {
      ...
      status: form.status, published_at: form.status === 'published' ? (editing?.published_at ?? new Date().toISOString()) : null,
    };
    const request = editing ? supabase.from('news_posts').update(payload).eq('id', editing.id) : supabase.from('news_posts').insert(payload);
  ...
  const editPost = (post: NewsPost) => {
    setEditing(post);
    setForm({ title: post.title, slug: post.slug, excerpt: post.excerpt, content: post.content, category: post.category, cover_image_url: post.cover_image_url || '', seo_title: post.seo_title || '', seo_description: post.seo_description || '', status: post.status });
    setError(''); setMessage('');
