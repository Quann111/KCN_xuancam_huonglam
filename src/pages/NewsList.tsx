...
export default function NewsList() {
  ...
  useEffect(() => { void (async () => { const { data, error: requestError } = await supabase.from('news_posts').select('*').eq('status', 'published').order('published_at', { ascending: false }); if (requestError) setError('Chưa thể tải danh sách tin tức. Vui lòng thử lại sau.'); else setPosts((data ?? []) as NewsPost[]); setLoading(false); })(); }, []);
